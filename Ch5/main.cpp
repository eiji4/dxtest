...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
	...
	// Create texture
	{
		...
		// WriteToSubresource method
		{
			...
			//result = _dev->CreateCommittedResource(&texHeapProp, D3D12_HEAP_FLAG_NONE, &texResDesc, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, nullptr, IID_PPV_ARGS(&texBuff));
			...
			//srvHeapDesc.NumDescriptors = 2; // テクスチャのUPの異なるやりかた用にヒープの領域で2つのDescriptorが入れられるようにしてみた。
			//srvHeapDesc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
			//result = _dev->CreateDescriptorHeap(&srvHeapDesc, IID_PPV_ARGS(&texDescHeap));
		...
		// CopyTextureRegion Method		
		{
			...
			src.PlacedFootprint.Footprint.Width = 100;
			//src.PlacedFootprint.Footprint.Height = texmeta.height;
			src.PlacedFootprint.Footprint.Height = 100;
			...
			_cmdList->CopyTextureRegion(&dst, 5, 1, 0, &src, nullptr);
	...
	// Create root signature
	{
		...
		rootParm.ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;
